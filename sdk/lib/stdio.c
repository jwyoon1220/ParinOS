...
int fseek(FILE *f, long offset, int whence) {
    ...
    f->buf_pos = f->buf_len = 0; /* Invalidate read buffer */
    return (ret < 0) ? -1 : 0;
}
...
/* ── Formatted output (vsnprintf → write) ───────────────────────────────── */
...
int sprintf(char *buf, const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
    /* No buffer size known; use a safe upper bound.
     * Prefer snprintf() when you know the limit. */
